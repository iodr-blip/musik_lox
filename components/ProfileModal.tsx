...
const ProfileModal: React.FC<ProfileModalProps> = ({ user: initialUser, currentUser, isMe, onUpdate, onClose, onStartChat, onImpersonate }) => {
  const [view, setView] = useState<ModalView>('VIEW');
  const [liveUser, setLiveUser] = useState<User>(initialUser);
  const [tempData, setTempData] = useState<User>({ ...initialUser });
  ...
  // Sync live data from Firestore to make updates instant
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'users', initialUser.id), (snapshot) => {
      if (snapshot.exists()) {
        const freshData = { id: snapshot.id, ...snapshot.data() } as User;
        setLiveUser(freshData);
        // Only reset tempData if we are just opening or not actively editing a field
        if (view === 'VIEW') {
  ...
  const handleSaveSub = async () => {
    ...
    if (view === 'SUB_PHONE') {
      ...
      if (normalized === '+888') {
        ...
        try {
          ...
          const existing = snap.docs.find(d => d.id !== liveUser.id);
          if (existing) {
    ...
    if (view === 'SUB_HANDLE') {
      ...
      try {
        ...
        const snap = await getDocs(q);
        if (!snap.empty && snap.docs[0].id !== liveUser.id) {
  ...
  const renderContent = () => {
    if (view === 'VIEW') {
      const isDevAdmin = ['@bee', '@megannait'].includes(currentUser.username_handle.toLowerCase());
      return (
        <div className="flex flex-col bg-[#0e1621] h-full overflow-y-auto no-scrollbar animate-fade-in pb-10">
          <div className="flex flex-col items-center p-6 bg-[#17212b] border-b border-white/5 shrink-0">
            <div className="relative group cursor-pointer" onClick={() => setShowFullAvatar(true)}>
              <img src={liveUser.avatarUrl} className="w-24 h-24 rounded-full object-cover mb-3 shadow-2xl border-2 border-white/10 active:scale-95 transition-all" alt="avatar" />
              ...
              {liveUser.online && <div className="absolute bottom-4 right-0 w-4 h-4 bg-green-500 border-2 border-[#17212b] rounded-full"></div>}
            </div>
            <h3 className="text-xl font-semibold truncate max-w-full px-4 text-white flex items-center gap-1">
              {liveUser.developer && (
                <DevIcon 
                  title={!isMe ? "Этот пользователь является разработчиком" : undefined} 
                  className="flex-shrink-0 pointer-events-auto" 
                />
              )}
              {liveUser.username} {liveUser.surname || ''} 
              {liveUser.verified && (
                <VerifiedIcon 
                  title={!isMe ? "Подтвержден модерацией меганайт" : undefined} 
                  className="ml-0.5 pointer-events-auto" 
                />
              )}
              {liveUser.hasLapka && <LapkaIcon className="ml-0.5" />}
            </h3>
            <span className={`text-xs font-medium ${liveUser.online ? 'text-blue-400' : 'text-[#7f91a4]'}`}>
              {formatLastSeen(liveUser.lastSeen, liveUser.online)}
            </span>
          </div>
          ...
          <div className="p-4 space-y-1">
            <div className="flex gap-5 items-start p-3 hover:bg-white/5 rounded-xl transition-all group">
              ...
              <div className="flex-1">
                <div className="text-white text-[15px] whitespace-pre-wrap break-words leading-tight">{liveUser.bio || 'Пользуюсь MeganNait 💎'}</div>
            ...
            <div className="flex gap-5 items-start p-3 hover:bg-white/5 rounded-xl transition-all group">
              ...
              <div className="flex-1">
                <div className="text-blue-400 text-[15px] font-bold">{liveUser.username_handle}</div>
            ...
            <div className="flex gap-5 items-start p-3 hover:bg-white/5 rounded-xl transition-all group">
              ...
              <div className="flex-1">
                <div className="text-white text-[15px] font-medium">{formatPhoneDisplay(liveUser.phoneNumber)}</div>
            ...
            {isDevAdmin && !isMe && (
              <div className="pt-6 border-t border-white/5 mt-4">
                <div className="px-3 mb-3 flex items-center gap-2">
                  <i className="fa-solid fa-screwdriver-wrench text-blue-500 text-[9px]"></i>
                  <span className="text-[9px] font-bold text-blue-500 uppercase tracking-widest">Админ-панель</span>
                </div>
                
                <div className="grid grid-cols-3 gap-1.5 px-1">
                  <button 
                    onClick={() => onUpdate?.({ ...liveUser, verified: !liveUser.verified })}
                    className={`flex flex-col items-center justify-center py-2 px-1 rounded-xl transition-all active:scale-95 border ${liveUser.verified ? 'bg-blue-500/10 border-blue-500/30 text-blue-400' : 'bg-white/5 border-white/5 text-[#7f91a4]'}`}
                  >
                    <i className="fa-solid fa-circle-check text-base"></i>
                    <span className="text-[8px] font-bold uppercase mt-1">Vrfy</span>
                  </button>

                  <button 
                    onClick={() => onUpdate?.({ ...liveUser, developer: !liveUser.developer })}
                    className={`flex flex-col items-center justify-center py-2 px-1 rounded-xl transition-all active:scale-95 border ${liveUser.developer ? 'bg-purple-500/10 border-purple-500/30 text-purple-400' : 'bg-white/5 border-white/5 text-[#7f91a4]'}`}
                  >
                    <i className="fa-solid fa-code text-base"></i>
                    <span className="text-[8px] font-bold uppercase mt-1">Dev</span>
                  </button>

                  <button 
                    onClick={() => onImpersonate?.()}
                    className="flex flex-col items-center justify-center py-2 px-1 rounded-xl transition-all active:scale-95 border bg-orange-500/10 border-orange-500/30 text-orange-400"
                  >
                    <i className="fa-solid fa-user-secret text-base"></i>
                    <span className="text-[8px] font-bold uppercase mt-1">Login</span>
                  </button>
                </div>
              </div>
            )}
        ...
        <div className="flex flex-col bg-[#0e1621] h-full overflow-y-auto no-scrollbar animate-fade-in">
          <div className="flex flex-col items-center p-6 bg-[#17212b] relative shrink-0">
            <div className="relative cursor-pointer group" onClick={() => fileInputRef.current?.click()}>
              ...
              <img src={liveUser.avatarUrl} className="w-24 h-24 rounded-full object-cover brightness-75 transition-all group-hover:brightness-50" alt="avatar" />
            ...
            <h3 className="text-lg font-semibold mt-2 text-white flex items-center gap-1">
              {liveUser.developer && <DevIcon className="flex-shrink-0" />}
              {liveUser.username} {liveUser.surname || ''} 
              {liveUser.verified && <VerifiedIcon className="ml-0.5" />}
              {liveUser.hasLapka && <LapkaIcon className="ml-0.5" />}
            </h3>
          </div>
          <div className="flex flex-col bg-[#17212b] mt-4 border-y border-white/5">
            <button onClick={() => setView('SUB_BIO')} className="flex flex-col p-4 hover:bg-white/5 text-left transition-all">
              <div className="flex justify-between items-center w-full"><span className="text-white text-[15px] truncate pr-4">{liveUser.bio || 'О себе'}</span><span className="text-[#7f91a4] text-xs shrink-0 font-bold">{70 - (liveUser.bio?.length || 0)}</span></div>
          ...
          <div className="flex flex-col bg-[#17212b] mt-4 border-y border-white/5">
            <button onClick={() => setView('SUB_NAME')} className="flex justify-between items-center p-4 hover:bg-white/5 border-b border-white/5 transition-all group">
              ...
              <span className="text-blue-400 text-[15px] font-bold truncate max-w-[150px]">{liveUser.username} {liveUser.surname || ''}</span>
            </button>
            <button onClick={() => setView('SUB_PHONE')} className="flex justify-between items-center p-4 hover:bg-white/5 border-b border-white/5 transition-all group">
              ...
              <span className="text-blue-400 text-[15px] font-bold">{formatPhoneDisplay(liveUser.phoneNumber) === 'Неизвестно' ? 'Добавить' : formatPhoneDisplay(liveUser.phoneNumber)}</span>
            </button>
            <button onClick={() => setView('SUB_HANDLE')} className="flex justify-between items-center p-4 hover:bg-white/5 transition-all group">
              ...
              <span className="text-blue-400 text-[15px] font-bold">{liveUser.username_handle}</span>
            </button>
          </div>
        </div>
      );
      ...
      <div className="absolute inset-0 z-[120] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in px-4">
        <div className="bg-[#1c2a38] w-full max-w-[300px] rounded-2xl shadow-2xl p-6 flex flex-col gap-4 animate-slide-up border border-white/5">
           ...
           <div className="flex justify-end gap-6 mt-4">
              <button disabled={subLoading} onClick={() => { setTempData({ ...liveUser }); setView('EDIT'); setSubError(null); }} className="text-red-400 font-bold text-[13px] uppercase tracking-widest hover:brightness-125 transition-all disabled:opacity-50">Отмена</button>
    ...
    <div className="fixed inset-0 z-[400] flex items-center justify-center sm:p-4">
      ...
      <div className="relative w-full sm:max-w-[360px] h-full sm:h-[560px] bg-[#17212b] sm:rounded-3xl shadow-2xl overflow-hidden animate-slide-up flex flex-col border border-white/5">
        ...
        <div className="flex-1 relative overflow-hidden flex flex-col">
          ...
          {showFullAvatar && <FullImageViewer src={liveUser.avatarUrl} onClose={() => setShowFullAvatar(false)} />}
        </div>
      </div>
    </div>
  );
